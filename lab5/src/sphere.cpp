  ...
  Eigen::MatrixXi & NF)
{
  ...
  double THETA_INCREMENT = M_PI / num_faces_v;  // inclination level increment - [0, pi]
  double PHI_INCREMENT = (2 * M_PI) / num_faces_u;  // azimuthal angle increment - [0, 2pi]

  // Projection - split png equally lengthwise and widthwise
  double U_INCREMENT = 1.0 / (num_faces_u + 1); // horizontal
  double V_INCREMENT = 1.0 / (num_faces_v + 1); // vertical
  ...
  for (int row = 0; row < num_faces_v + 1; row++) { // + 1 to get the lower and upper vertices
    for (int col = 0; col < num_faces_u + 1; col++) { // + 1 to get the left and right vertices
      ...
      UV.row(vertex) = Vector2d(U_INCREMENT * col, V_INCREMENT * (num_faces_v + 1 - row));
    }
  }
